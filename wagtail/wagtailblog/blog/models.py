...
from wagtailmedia.blocks import MediaChooserBlock  # 新增导入
...
class VideoBlock(blocks.StructBlock):
	"""视频内容块（使用wagtailmedia）"""
	media = MediaChooserBlock(label='选择视频')
	caption = blocks.CharBlock(required=False, label='视频标题')
	
	class Meta:
		icon = 'media'
		label = '视频块'
		template = 'blog/blocks/media_block.html'
...
class BlogPage(Page):
	...
	# 修改 StreamField 定义，将 video_block 替换为新版本
	body = StreamField([
	...
	# 在blog/models.py的BlogPage类中添加
	def serve(self, request, *args, **kwargs):
		"""重写serve方法，为前端加载MongoDB内容"""
		# 加载MongoDB内容
		...
		if content and 'body' in content:
			...
			# 设置body内容
			self.body = StreamValue(stream_block, content['body'], is_lazy=True)
