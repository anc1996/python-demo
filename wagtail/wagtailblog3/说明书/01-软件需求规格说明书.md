...
其主要功能范畴包括：
* **智能内容创作与管理**: 提供强大的后台，支持多格式内容混合编排。**集成 LLM（大语言模型）能力，提供 AI 辅助标题生成、摘要润色及内容纠错功能。支持 Mermaid.js 语法，实现流程图、时序图的代码化绘制。**
...
* **高可扩展性架构**: 采用 **混合存储架构（MySQL 存储关系数据 + MongoDB 存储非结构化正文）**，支持媒体文件对象存储（MinIO），利用缓存（Redis）和异步任务队列（Celery）优化性能。
...
* **LLM**: Large Language Model，大语言模型（如 OpenAI GPT），用于辅助内容生成。
* **Mermaid**: 一种基于 JavaScript 的图表绘制工具，通过简单的文本语法生成图表。
* **Wagtail AI**: Wagtail 的 AI 扩展包，集成了 LLM 接口。
...
* **数据存储**:
    ...
    - **关系型/元数据存储**: MySQL (`mysqlclient`) —— 存储页面层级、用户、评论、分类标签等结构化数据。
    - **内容存储**: **MongoDB (`pymongo`)** —— **存储博客文章的 StreamField 正文数据（JSON 格式），实现内容与元数据的分离，便于非结构化检索和扩展。**
...
* **FR-1.1: 文章创建与编辑**:
    ...
    * **AI 辅助编辑**:
      - **智能标题**: 编辑界面集成 AI 标题生成面板 (`AITitleFieldPanel`)，可根据内容自动建议 SEO 友好的标题。
      - **智能简介**: 提供 AI 简介生成面板 (`AIDescriptionFieldPanel`)，自动提取文章摘要或润色简介 (`intro`)。
      - **SEO 优化**: 支持 AI 辅助生成 SEO 标题和搜索描述 (`promote_panels`)。
* **FR-1.2: 灵活的内容构造 (StreamField)**:
    
    * 文章正文 (`body`) **存储于 MongoDB**，使用 `StreamField` 实现，支持以下内容块 (`apps/blog/blocks.py`):
    
        - **富文本块 (`RichTextBlock`)**: 支持加粗、列表、链接等，**并集成 AI 文本修正与续写功能 (`features=['ai']`)**。
        - **Mermaid 图表块 (`MermaidBlock`)**: **新增支持。允许用户输入 Mermaid 语法代码（如 `graph TD; A-->B;`），前端自动渲染为流程图、时序图或甘特图。**
        - **Markdown 块 (`MarkdownBlock`)**: 支持标准 Markdown、**数学公式 (KaTeX)** 及代码高亮。
        - **自定义表格块 (`CustomTableBlock`)**: 支持单元格合并与样式定义的增强型表格。
...
- 系统通过 `ai_backends.OpenAICompatibleBackend` 集成外部 LLM 服务。
- 支持配置自定义 `BASE_URL` 以兼容 OpenAI 格式的第三方模型（如本地部署模型或国内大模型代理）。
- 配置项需在环境变量 (`.env`) 中定义，包括 `OPENAI_API_KEY` 和 `WAGTAIL_AI_BACKENDS` 设置。
