...
SECRET_KEY = "django-insecure-rqukw1zt6&99zt6b4^oc#!b^b$8wu4cv8vp=&=wb^k@ooj$vi7"
...
INSTALLED_APPS = [
    ...
    'book.apps.BookConfig',# 注册图书应用,指定book文件下，app.py的BookConfig类
    # 'book', # 也可以这种方式注册
    'login.apps.LoginConfig',# 注册登录应用,指定login文件下，app.py的LoginConfig类
    # ‘子应用名’,
...
MIDDLEWARE = [
    ...
    # 自己注册中间件,目前来自book/middleware.py
    "book.middleware.simple_middleware1",
...
TEMPLATES = [
    ...
    },
    {
        ...
        "OPTIONS": {
            ...
            "environment": "book.jinja2_env.environment", # 一定要设置，指向一个返回 Jinja2 环境的可调用对象。
            "context_processors": [
                "django.template.context_processors.debug",
                "django.template.context_processors.request",
                "django.contrib.auth.context_processors.auth",
...
DATABASES = {
    ...
    "default": {
        ...
        'USER': 'root',  # 用户名
        'PASSWORD': '123456',  # 密码
        'NAME': 'bookmanager',  # 指定数据库
    },
    # sqlite 主要是一个嵌入式关系行数据库。一般小行数据库使用sqlite，如移动端
