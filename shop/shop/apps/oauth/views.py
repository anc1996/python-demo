...
class QQAuthURLView(View):
    ...
    def get(self,request):
        # 1、获取next参数
        next=request.GET.get('next') # xxxxx/xxx/?next=地址
...
class QQAuthUserView(View):
    ...
    def get(self,request):
        ...
        try:
            # 2、QQ第二步：通过Authorization Code获取Access Token
            access_token=oauth.get_access_token(code)
            # 3、QQ第三步：通过access_token获取openid
            openid=oauth.get_open_id(access_token)
        except Exception as e:
            logger.error('错误信息:%s'% e )
            return HttpResponseServerError('OAuth2.0认证失败')

        # 4、使用openid 该qq用户是否绑定过美多商城的用户
        try:
            # 通过openid查询对应的用户
            oauth_qq_user=OAuthQQUser.objects.get(openid=openid)
        except OAuthQQUser.DoesNotExist as e:
            # 5、openid未绑定用户，oauth_callback.html的access_token_openid网页记录openid
            ...
            # 5、openid已绑定用户,oauth_user.user从qq模型类找到对应的user模型类对象
