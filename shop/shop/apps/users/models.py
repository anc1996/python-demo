...
class User(AbstractUser):
     # 自定义用户模型类,在自定义用户模型类时要到settings.py中设置AUTH_USER_MODEL = 'users.User'，
     # 否则依赖系统自带用户模型类global_settings
     '''
         继承父类：AbstractUser父类
...
class Address(BaseModel):
    """用户地址"""
    # on_delete=models.CASCADE表示当User模型中的记录被删除时，与之关联的Address模型中的记录也会被自动删除。
    ...
    class Meta:
        ...
        # 根据根据updatetime字段倒序更新,updatetime字段在BaseModel模型中
