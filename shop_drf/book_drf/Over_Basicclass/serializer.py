...
class PeopleInfoSerializer(serializers.Serializer):
    ...
    # 方法一：PrimaryKeyRelatedField，此字段将被序列化为关联对象的主键。返回是 id
    ...
    def get_gender_text(self, obj):
        return dict(PeopleInfoSerializer.GENDER_CHOICES).get(obj.gender, 'unknown')
...
class BookModelSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ('id',)
        # 还可以对字段进行添加和修改
        extra_kwargs={ 'readcount': {'min_value': 10,'required': True},'commentcount':{'min_value':5,'required': True} }
