...
def fresh():
   ...
   # 如果我们在这里刷新令牌，我们已经有一段时间没有验证用户密码了，因此请将新创建的访问令牌标记为非新令牌
    """
        1、Fresh 访问令牌：当用户通过提供用户名和密码进行身份验证时，服务器会生成一个“fresh”访问令牌。这个 fresh 访问令牌可以访问所有路由，包括那些需要高安全级别的敏感操作（如更改电子邮件地址、修改密码等）。
        2、非 Fresh 访问令牌：随着时间的推移，fresh 访问令牌会过期或被视为不再 fresh。此时，用户仍然可以使用这个访问令牌访问大部分非敏感的路由，但无法再访问那些需要 fresh 访问令牌的敏感路由。
