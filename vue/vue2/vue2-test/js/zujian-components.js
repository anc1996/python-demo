...
// 局部组件定义a
var jvbuzujian_a={
    template: '<div>局部组件定义a,{{age}},局部使用全局变更量{{pro}},<button @click="upload">上传给信息父组件</button><jvbuzujian_c></jvbuzujian_c></div>',
    data:function () {
        return {age:18}
    },
...
Vue.component(
    ...
    'component-all', {
        ...
        template:'<div>全局组件定义,{{name}}.' +
                    ...
                    '<button @click="add">点击add方法</button> ' +
                '</div>',
        // 将局部组件注册到全局组件中
        components:{
            jvbuzujian_a,
